...
sf::RenderWindow window(sf::VideoMode(WinSize, WinSize), "rotations");
...
void organize() {
	...
	for (int i = 0; i < type.size(); i++) {
		if (type[i] == 0) {
			pos.push_back(p1[i][2]);
		}
		else if (type[i] == 1) {
			pos.push_back(( p1[i][2] + p2[i][2]) / 2);
		}
		else {
			pos.push_back(( p3[i][2] + p2[i][2] + p1[i][2] ) / 3);
		}
	}
